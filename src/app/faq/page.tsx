...
const faqs = [
  ...
  {
    question: "Does Printway offer pre-press structural or graphic design support?",
  ...
  {
    question: "How does Printway handle delivery across Sri Lanka?",
...
export default function FAQPage() {
  return (
    <div className="pt-32 pb-0 bg-[#020202] min-h-screen relative overflow-hidden">
      {/* Cinematic ambient background */}
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[400px] bg-gradient-to-r from-[#00AEEF]/10 via-[#EC008C]/10 to-transparent rounded-[100%] blur-[120px] pointer-events-none" />
      ...
      <div className="container mx-auto px-4 md:px-6 mb-32 relative z-10 max-w-4xl">
        <div className="text-center mb-20">
          ...
          >
            Frequently Asked <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#00AEEF] to-white/40">Questions</span>
          </motion.h1>
        ...
        <motion.div 
          ...
          className="bg-[#0a0a0a]/80 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-8 md:p-14 shadow-2xl relative overflow-hidden"
        >
          {/* Subtle inner top highlight */}
          <div className="absolute inset-x-0 top-0 h-[1px] bg-gradient-to-r from-transparent via-[#00AEEF]/50 to-transparent" />
          ...
          <Accordion className="w-full space-y-6">
            {faqs.map((faq, index) => (
              <AccordionItem 
                key={index} 
                value={`item-${index}`} 
                className="border border-white/5 bg-[#111] hover:bg-[#151515] transition-colors rounded-2xl px-6 data-[state=open]:bg-white/5 data-[state=open]:border-white/20"
              >
                <AccordionTrigger className="text-lg md:text-xl font-bold py-6 text-white hover:text-[#00AEEF] data-[state=open]:text-[#00AEEF] transition-colors hover:no-underline text-left">
                  {faq.question}
                </AccordionTrigger>
